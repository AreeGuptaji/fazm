...
echo "=== Full Fazm Cleanup ==="

# Kill the app if running
echo "Killing app..."
pkill -9 "Fazm" 2>/dev/null || true

# Remove apps from Applications (all variants)
for app in "/Applications/Fazm.app" "/Applications/Fazm Dev.app" "/Applications/Omi Computer.app" "/Applications/Omi Beta.app" "/Applications/Omi Dev.app"; do
    if [ -d "$app" ]; then
        echo "Removing $app..."
        rm -rf "$app"
    fi
...
for app in "build/Fazm.app" "build/Fazm Dev.app" "build/Omi Computer.app" "build/Omi Beta.app" "build/Omi Dev.app"; do
    if [ -d "$app" ]; then
        echo "Removing $app..."
        rm -rf "$app"
    fi
