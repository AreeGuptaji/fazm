...
class ChatToolExecutor {
    ...
    static func formatKGAsText(nodes: [LocalKGNodeRecord], edges: [LocalKGEdgeRecord]) -> String {
        ...
        let grouped = Dictionary(grouping: nodes, by: { $0.nodeType })
        for (type, typeNodes) in grouped.sorted(by: { $0.key < $1.key }) {
            let names = typeNodes.map { $0.label }.joined(separator: ", ")
            lines.append("\(type.capitalized)s: \(names)")
        }

        // Format relationships
        for edge in edges {
            if let source = nodeLabels[edge.sourceNodeId],
               let target = nodeLabels[edge.targetNodeId] {
                lines.append("\(source.label) \(edge.label) \(target.label)")
            }
        }
    ...
    static func retainKGToHindsight(nodes: [LocalKGNodeRecord], edges: [LocalKGEdgeRecord]) async {
        ...
        let payload: [String: Any] = [
            ...
            "params": [
                ...
                "arguments": [
                    "content": text,
                    "context": "onboarding_knowledge_graph",
                    "tags": ["onboarding", "knowledge_graph"]
                ]
            ]
        ]

        guard let jsonData = try? JSONSerialization.data(withJSONObject: payload) else {
            log("retainKGToHindsight: failed to serialize JSON")
            return
        }
        ...
        do {
            ...
            if statusCode >= 200 && statusCode < 300 {
                log("retainKGToHindsight: success (\(nodes.count) nodes, \(edges.count) edges)")
            } else {
                let body = String(data: data, encoding: .utf8) ?? ""
                log("retainKGToHindsight: HTTP \(statusCode): \(body.prefix(200))")
            }
        } catch {
            log("retainKGToHindsight: \(error.localizedDescription)")
        }
    }
}
